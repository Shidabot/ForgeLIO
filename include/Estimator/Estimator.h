...
public:
	...
	*/
	Estimator(const float& filter_corner, const float& filter_surf,
			  bool save_map_on_shutdown = true,
			  const std::string& map_file_path = "/tmp/lio_livox_global_map.pcd",
			  float save_map_leaf_size = 0.2f,
...
private:
	struct MapUpdate {
		...
		pcl::PointCloud<PointType>::Ptr nonfeature;
		Eigen::Matrix4d transform;

		MapUpdate()
				: corner(new pcl::PointCloud<PointType>),
				  surf(new pcl::PointCloud<PointType>),
				  nonfeature(new pcl::PointCloud<PointType>),
				  transform(Eigen::Matrix4d::Identity()) {}
	};

	void appendToGlobalMap(const pcl::PointCloud<PointType>::Ptr& corner,
	...
	std::deque<MapUpdate, Eigen::aligned_allocator<MapUpdate>> mapUpdateQueue;
	bool stopMapThread = false;
	std::thread threadMap;

	bool saveMapOnShutdown;
	std::string mapFilePath;
	float saveMapLeafSize;
	pcl::PointCloud<PointType>::Ptr globalMapForSave;
	std::size_t nextGlobalMapCompaction = 1000000;
	...
	double lidarHuberDelta = 0.1;
	double lidarOutlierThreshold = 1.0;
};
