...
class PCSeg
{
    ...
    private:
    // Reused by DoSeg to avoid several heap allocations on every scan.
    std::vector<float> downsampledPoints_;
    std::vector<int> pointVoxelIndices_;
    std::vector<int> voxelPointIndices_;
    std::vector<int> groundLabels_;
    std::vector<float> aboveGroundPoints_;
    std::vector<int> aboveGroundIndices_;
    std::vector<int> pointLabels_;
    std::vector<int> aboveGroundLabels_;
