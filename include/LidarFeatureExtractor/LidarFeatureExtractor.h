...
public:
    ...
    void FeatureExtract_Mid(pcl::PointCloud<pcl::PointXYZINormal>::Ptr &msg,
                                                   ...
                                                   pcl::PointCloud<PointType>::Ptr& laserSurfFeature);
private:
    /** \brief Persistent segmentation instance and work buffers.
      *
      * Keeping these objects alive avoids allocating the 12 MB voxel label grid
      * and the per-point input/output arrays for every LiDAR frame.
      */
    PCSeg pcseg_;
    std::vector<int> segment_labels_;
    std::vector<float> segment_data_;
    pcl::KdTreeFLANN<PointType>::Ptr feature_kdtree_;
