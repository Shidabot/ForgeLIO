...
struct ResidualBlockInfo
{
	ResidualBlockInfo(ceres::CostFunction *_cost_function,
					...
					bool _owns_cost_function = true)
					: cost_function(_cost_function),
					  loss_function(_loss_function),
					  owns_cost_function(_owns_cost_function),
	...
	// Cost functions already added to a ceres::Problem are owned by that
	// problem.  Marginalization-only factors remain owned here.
	bool owns_cost_function;
...
public:
	...
	bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const override{
		...
		for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++)
		{
			...
			// Pose blocks are optimized as six ordinary Euclidean parameters:
			// [translation, SO(3) logarithm].  The Jacobians accumulated during
			// marginalization are therefore derivatives with respect to those exact
			// coordinates.  Replacing the rotation-vector difference with a group
			// composition here changes both its sign and its local Jacobian, while
			// the Jacobian returned below remains unchanged.  Use the same Euclidean
			// delta for every block to keep the prior and its linearization consistent.
      ...
      T a012 = ceres::sqrt(
              ...
              * ((P_to_Map(0) - lpa(0)) * (P_to_Map(1) - lpb(1)) - (P_to_Map(0) - lpb(0)) * (P_to_Map(1) - lpa(1)))
              ...
              + ((P_to_Map(1) - lpa(1)) * (P_to_Map(2) - lpb(2)) - (P_to_Map(1) - lpb(1)) * (P_to_Map(2) - lpa(2)))
                * ((P_to_Map(1) - lpa(1)) * (P_to_Map(2) - lpb(2)) - (P_to_Map(1) - lpb(1)) * (P_to_Map(2) - lpa(2))));
      T ld2 = a012 / T(l12);
      residual[0] = T(sqrt_information(0)) * ld2;
...
struct Cost_NavState_IMU_Plan
{
    ...
    bool operator()(const T *PRi, T *residual) const {
      ...
      residual[0] = T(sqrt_information(0)) * pd2;
