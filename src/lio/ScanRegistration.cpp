...
void lidarCallBackHorizon(const livox_ros_driver::CustomMsgConstPtr &msg) {
  ...
  if (!msg || msg->points.empty()) {
    ROS_WARN_THROTTLE(1.0, "Received an empty Livox point cloud; skipping this scan.");
    return;
  }
  ...
  if (laserCloud->empty()) {
    ROS_WARN_THROTTLE(1.0, "No valid Livox points remained after preprocessing; skipping this scan.");
    return;
  }
...
void lidarCallBackHAP(const livox_ros_driver::CustomMsgConstPtr &msg) {
  ...
  if (!msg || msg->points.empty()) {
    ROS_WARN_THROTTLE(1.0, "Received an empty HAP point cloud; skipping this scan.");
    return;
  }
  ...
  if (laserCloud->empty()) {
    ROS_WARN_THROTTLE(1.0, "No valid HAP points remained after preprocessing; skipping this scan.");
    return;
  }
...
void lidarCallBackPc2(const sensor_msgs::PointCloud2ConstPtr &msg) {
    ...
    if (laser_cloud->empty()) {
        ROS_WARN_THROTTLE(1.0, "Received an empty PointCloud2 message; skipping this scan.");
        return;
    }
    ...
    for (uint64_t i = 0; i < laser_cloud->points.size(); i++)
    {
        ...
        p_custom.normal_x = 0.0f;
        p_custom.normal_y = 0.0f;
        p_custom.normal_z = 0.0f;
        p_custom.curvature = 0.0f;
    ...
    if (laser_cloud_custom->empty()) {
        ROS_WARN_THROTTLE(1.0, "No valid PointCloud2 points remained after preprocessing; skipping this scan.");
        return;
    }
