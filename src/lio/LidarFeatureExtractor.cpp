...
LidarFeatureExtractor::LidarFeatureExtractor(int n_scans,int NumCurvSize,float DistanceFaraway,int NumFlat,
  ...
  feature_kdtree_.reset(new pcl::KdTreeFLANN<PointType>);
}
...
void LidarFeatureExtractor::FeatureExtract_with_segment(const livox_ros_driver::CustomMsgConstPtr &msg,
  ...
  if (!msg || msg->points.empty()) {
    ROS_WARN_THROTTLE(1.0, "Received an empty Livox point cloud; skipping feature extraction.");
    return;
  }
  ...
  const std::size_t dnum = msg->points.size();
  segment_data_.clear();
  segment_data_.reserve(dnum * 4);
  ...
  for(const auto& p : msg->points){
    ...
    point.normal_x = timeSpan > 0.0
                     ? ros::Time().fromNSec(p.offset_time).toSec() / timeSpan
                     : 0.0;
    point.normal_y = _int_as_float(line_num);
    laserCloud->push_back(point);

    segment_data_.push_back(point.x);
    segment_data_.push_back(point.y);
    segment_data_.push_back(point.z);
    segment_data_.push_back(point.intensity);
  }

  const std::size_t point_num = laserCloud->size();
  if (point_num == 0) {
    ROS_WARN_THROTTLE(1.0, "No valid Livox points remained after input filtering.");
    return;
  }
  segment_labels_.assign(point_num, 0);
  if (pcseg_.DoSeg(segment_labels_.data(), segment_data_.data(),
                   static_cast<int>(point_num)) != 0) {
    ROS_WARN_THROTTLE(1.0, "Point-cloud segmentation failed; keeping the current scan unfiltered.");
    std::fill(segment_labels_.begin(), segment_labels_.end(), 0);
  }
  ...
  threads.reserve(N_SCANS);
  for(int i=0; i<N_SCANS; ++i){
    threads.emplace_back(&LidarFeatureExtractor::detectFeaturePoint3, this,
  ...
  for(std::size_t i=0; i<cloud_num; ++i){
    ...
    if( segment_labels_[i] > 9 && dis < 50*50){
...
void LidarFeatureExtractor::FeatureExtract_with_segment_hap(const livox_ros_driver::CustomMsgConstPtr &msg,
  ...
  laserNonFeature->clear();
  if (!msg || msg->points.empty()) {
    ROS_WARN_THROTTLE(1.0, "Received an empty HAP point cloud; skipping feature extraction.");
    return;
  }
  laserCloud->reserve(15000*N_SCANS);
  for(auto & ptr : vlines){
  ...
  const std::size_t point_num = laserCloud->size();
  if (point_num == 0) {
    ROS_WARN_THROTTLE(1.0, "No valid HAP points remained after input filtering.");
    return;
  }
  segment_labels_.assign(point_num, 0);
  if (pcseg_.DoSeg(segment_labels_.data(), segment_data_.data(),
                   static_cast<int>(point_num)) != 0) {
    ROS_WARN_THROTTLE(1.0, "Point-cloud segmentation failed; keeping the current HAP scan unfiltered.");
    std::fill(segment_labels_.begin(), segment_labels_.end(), 0);
  }
...
void LidarFeatureExtractor::detectFeaturePoint2(pcl::PointCloud<PointType>::Ptr& cloud,
  ...
  pointsLessFlat->clear();
  pointsNonFeature->clear();
  feature_kdtree_->setInputCloud(cloud);
  ...
  for(int i = 5; i < cloudSize - 5; i = i+stride) {
    ...
    if(feature_kdtree_->nearestKSearch(cloud->points[i], num_near,
                                      _pointSearchInd, _pointSearchSqDis) < num_near){
      continue;
    }
...
void LidarFeatureExtractor::FeatureExtract(const livox_ros_driver::CustomMsgConstPtr &msg,
  ...
  if (!pcl_isfinite(p.x) || !pcl_isfinite(p.y) || !pcl_isfinite(p.z)) continue;
//  if(p.x < 0.01) continue;
  point.x = p.x;
  point.y = p.y;
  point.z = p.z;
  ...
  threads.reserve(N_SCANS);
  for(int i=0; i<N_SCANS; ++i){
  threads.emplace_back(&LidarFeatureExtractor::detectFeaturePoint, this,
...
void LidarFeatureExtractor::FeatureExtract_Mid(pcl::PointCloud<pcl::PointXYZINormal>::Ptr &msg,
    ...
    laserSurfFeature->clear();
    if (!msg || msg->empty()) {
        ROS_WARN_THROTTLE(1.0, "Received an empty Mid-series point cloud; skipping feature extraction.");
        return;
    }
    for(auto & ptr : vlines){
        ptr->clear();
    ...
    for(int i=0; i<cloud_num; ++i){
        msg->points[i].normal_z = 0.0f;
        int line_idx = std::round(msg->points[i].normal_y);
        if(line_idx < 0 || line_idx >= N_SCANS) continue;
