...
Estimator::Estimator(const float& filter_corner, const float& filter_surf,
                     bool save_map_on_shutdown,
                     const std::string& map_file_path,
                     float save_map_leaf_size,
                     double lidar_huber_delta,
                     double lidar_outlier_threshold){

  saveMapOnShutdown = save_map_on_shutdown;
  mapFilePath = map_file_path;
  saveMapLeafSize = save_map_leaf_size;
  lidarHuberDelta = std::isfinite(lidar_huber_delta) &&
                    lidar_huber_delta >= 1e-4
                        ? lidar_huber_delta
                        : 0.1;
  lidarOutlierThreshold = std::isfinite(lidar_outlier_threshold) &&
                          lidar_outlier_threshold >= lidarHuberDelta
                              ? lidar_outlier_threshold
                              : std::max(1.0, lidarHuberDelta);
  ...
  for(auto& p:laserCloudNonFeatureStack)
    p.reset(new pcl::PointCloud<PointType>);
  globalMapForSave.reset(new pcl::PointCloud<PointType>);
...
Estimator::~Estimator(){
  {
    std::lock_guard<std::mutex> locker(mtx_Map);
    stopMapThread = true;
  }
  cv_Map.notify_all();
  ...
  if(saveMapOnShutdown){
    saveGlobalMap();
  }
  delete map_manager;
}

void Estimator::appendToGlobalMap(const pcl::PointCloud<PointType>::Ptr& corner,
                                  const pcl::PointCloud<PointType>::Ptr& surf,
                                  const pcl::PointCloud<PointType>::Ptr& nonfeature){
  if(!saveMapOnShutdown){
    return;
  }

  pcl::PointCloud<PointType>::Ptr batch(new pcl::PointCloud<PointType>);
  batch->reserve(corner->size() + surf->size() + nonfeature->size());
  *batch += *corner;
  *batch += *surf;
  *batch += *nonfeature;
  if(batch->empty()){
  ...
  if(saveMapLeafSize > 0.0f){
    pcl::PointCloud<PointType>::Ptr filtered(new pcl::PointCloud<PointType>);
    pcl::VoxelGrid<PointType> filter;
    filter.setLeafSize(saveMapLeafSize, saveMapLeafSize, saveMapLeafSize);
    filter.setInputCloud(batch);
    filter.filter(*filtered);
    batch.swap(filtered);
  }

  globalMapForSave->reserve(globalMapForSave->size() + batch->size());
  *globalMapForSave += *batch;

  // Periodically merge duplicate voxels so long mapping sessions do not retain
  // every overlapping observation until shutdown.
  if(saveMapLeafSize > 0.0f &&
     globalMapForSave->size() >= nextGlobalMapCompaction){
    pcl::PointCloud<PointType>::Ptr compacted(new pcl::PointCloud<PointType>);
    pcl::VoxelGrid<PointType> filter;
    filter.setLeafSize(saveMapLeafSize, saveMapLeafSize, saveMapLeafSize);
    filter.setInputCloud(globalMapForSave);
    filter.filter(*compacted);
    globalMapForSave.swap(compacted);
    nextGlobalMapCompaction = globalMapForSave->size() + 1000000;
  }
}

bool Estimator::saveGlobalMap(){
  if(mapFilePath.empty()){
    ROS_ERROR("Global map path is empty; the map was not saved.");
    return false;
  }
  if(!globalMapForSave || globalMapForSave->empty()){
    ROS_WARN("Global map is empty; no PCD file was written.");
    return false;
  }
  ...
  if(saveMapLeafSize > 0.0f){
    ...
    *output = *globalMapForSave;
  }

  output->width = output->size();
  output->height = 1;
  if(pcl::io::savePCDFileBinaryCompressed(mapFilePath, *output) != 0){
    ROS_ERROR_STREAM("Failed to save the global map to " << mapFilePath
                     << ". Ensure that the parent directory exists and is writable.");
    return false;
  }

  ROS_INFO_STREAM("Saved global map with " << output->size()
                  << " points to " << mapFilePath);
  return true;
}

void Estimator::threadMapIncrement(){
  ...
  while(true){
    MapUpdate update;
    {
      ...
      cv_Map.wait(locker, [this]{ return stopMapThread || !mapUpdateQueue.empty(); });
      if(mapUpdateQueue.empty()){
        if(stopMapThread){
          break;
        }
        continue;
      }
      update = std::move(mapUpdateQueue.front());
      mapUpdateQueue.pop_front();
    }
    ...
    map_manager->featureAssociateToMap(update.corner,
                                         update.surf,
                                         update.nonfeature,
                                         laserCloudCorner,
                                         laserCloudSurf,
  ...
  // Flush the last partial batch so shutdown never loses the final frames.
  if(!laserCloudCorner_to_map->empty() || !laserCloudSurf_to_map->empty() ||
     !laserCloudNonFeature_to_map->empty()){
    map_manager->MapIncrement(laserCloudCorner_to_map,
                              laserCloudSurf_to_map,
                              laserCloudNonFeature_to_map,
                              transform);
...
void Estimator::processPointToLine(std::vector<ceres::CostFunction *>& edges,
  ...
  for (int i = 0; i < laserCloudCornerStackNum; i++) {
    ...
    if(!std::isfinite(_pointSel.x) || !std::isfinite(_pointSel.y) ||
       !std::isfinite(_pointSel.z)) continue;

    if(GlobalCornerMap[id].points.size() > 100) {
      _pointSearchInd.clear();
      _pointSearchSqDis.clear();
      const int neighbor_count =
          CornerKdMap[id].nearestKSearch(_pointSel, 5, _pointSearchInd,
                                         _pointSearchSqDis);
      
      if (neighbor_count >= 5 && _pointSearchInd.size() >= 5 &&
          _pointSearchSqDis.size() >= 5 &&
          std::isfinite(_pointSearchSqDis[4]) &&
          _pointSearchSqDis[4] < thres_dist) {
      ...
      if (saes.info() == Eigen::Success && saes.eigenvalues().allFinite() &&
          saes.eigenvectors().allFinite() &&
          saes.eigenvalues()[2] > 1e-12 &&
    ...
    if(laserCloudCornerLocal->points.size() > 20 ){
      _pointSearchInd2.clear();
      _pointSearchSqDis2.clear();
      const int neighbor_count =
          kdtreeLocal->nearestKSearch(_pointSel, 5, _pointSearchInd2,
                                      _pointSearchSqDis2);
      if (neighbor_count >= 5 && _pointSearchInd2.size() >= 5 &&
          _pointSearchSqDis2.size() >= 5 &&
          std::isfinite(_pointSearchSqDis2[4]) &&
          _pointSearchSqDis2[4] < thres_dist) {
...
void Estimator::processPointToPlan(std::vector<ceres::CostFunction *>& edges,
  ...
  for (int i = 0; i < laserCloudSurfStackNum; i++) {
    ...
    if(GlobalSurfMap[id].points.size() > 50) {
      ...
      if (neighbor_count >= 5 && _pointSearchInd.size() >= 5 &&
          _pointSearchSqDis.size() >= 5 && std::isfinite(_pointSearchSqDis[4]) &&
          _pointSearchSqDis[4] < 1.0) {
        debug_num1 ++;
        for (int j = 0; j < 5; j++) {
        ...
        bool planeValid = lio_livox::numerics::FitNormalizedPlane(_matA0, &plane);
        const double pa = plane(0);
        const double pb = plane(1);
        const double pc = plane(2);
        const double pd = plane(3);

        for (int j = 0; planeValid && j < 5; j++) {
    ...
    if (neighbor_count >= 5 && _pointSearchInd2.size() >= 5 &&
        _pointSearchSqDis2.size() >= 5 && std::isfinite(_pointSearchSqDis2[4]) &&
        _pointSearchSqDis2[4] < 1.0) {
      debug_num2++;
      for (int j = 0; j < 5; j++) { 
...
void Estimator::processNonFeatureICP(std::vector<ceres::CostFunction *>& edges,
  ...
  for (int i = 0; i < laserCloudNonFeatureStackNum; i++) {
    ...
    if(GlobalNonFeatureMap[id].points.size() > 100) {
      ...
      if (neighbor_count >= 5 && _pointSearchInd.size() >= 5 &&
          _pointSearchSqDis.size() >= 5 && std::isfinite(_pointSearchSqDis[4]) &&
          _pointSearchSqDis[4] < 1 * thres_dist) {
        for (int j = 0; j < 5; j++) {
    ...
    if(laserCloudNonFeatureLocal->points.size() > 20 ){
      ...
      if (neighbor_count >= 5 && _pointSearchInd2.size() >= 5 &&
          _pointSearchSqDis2.size() >= 5 && std::isfinite(_pointSearchSqDis2[4]) &&
          _pointSearchSqDis2[4] < 1 * thres_dist) {
        for (int j = 0; j < 5; j++) { 
...
void Estimator::EstimateLidarPose(std::list<LidarFrame>& lidarFrameList,
  ...
  *map_update.corner = *laserCloudCornerStack[0];
  *map_update.surf = *laserCloudSurfStack[0];
  *map_update.nonfeature = *laserCloudNonFeatureStack[0];
  map_update.transform = transformTobeMapped;
  {
    std::lock_guard<std::mutex> locker(mtx_Map);
    mapUpdateQueue.emplace_back(std::move(map_update));
  }
  cv_Map.notify_one();
  ...
  MapIncrementLocal(laserCloudCornerStack[0], laserCloudSurfStack[0],
                    laserCloudNonFeatureStack[0], transformTobeMapped);
}
...
void Estimator::Estimate(std::list<LidarFrame>& lidarFrameList,
  ...
  for(int iterOpt=0; iterOpt<max_iters; ++iterOpt){
    ...
    // Keep the robust scale in metres at the ROS interface and normalize it
    // by the same standard deviation used by the LiDAR residuals.  Robust
    // loss is required during normal two-frame LIO as well as initialization.
    ceres::LossFunction* loss_function =
        new ceres::HuberLoss(lidarHuberDelta / IMUIntegrator::lidar_m);
    ...
    for(int f=1; f<windowSize; ++f){
      ...
      Eigen::Matrix<double, 15, 15> sqrt_information_imu;
      if (!lio_livox::numerics::ComputeRegularizedSqrtInformation<15>(
              frame_curr->imuIntegrator.GetCovariance(),
              &sqrt_information_imu)) {
        ROS_WARN_THROTTLE(1.0,
                          "Skipping an IMU factor with invalid covariance.");
        continue;
      }
    ...
    int cntCorner = 0;
    int cntNon = 0;
    const double max_lidar_residual =
        windowSize == SLIDEWINDOWSIZE
            ? lidarOutlierThreshold
            : 3.0 * lidarOutlierThreshold;
    if(windowSize == SLIDEWINDOWSIZE) {
      thres_dist = 1.0;
      if(iterOpt == 0){
        for(int f=0; f<windowSize; ++f){
          ...
          for (auto &e : edgesLine[f]) {
            if(std::isfinite(vLineFeatures[f][cntFtu].error) &&
               std::fabs(vLineFeatures[f][cntFtu].error) <= max_lidar_residual){
          ...
          for (auto &e : edgesPlan[f]) {
            if(std::isfinite(vPlanFeatures[f][cntFtu].error) &&
               std::fabs(vPlanFeatures[f][cntFtu].error) <= max_lidar_residual){
              problem.AddResidualBlock(e, loss_function, para_PR[f]);
          ...
          for (auto &e : edgesNon[f]) {
            if(std::isfinite(vNonFeatures[f][cntFtu].error) &&
               std::fabs(vNonFeatures[f][cntFtu].error) <= max_lidar_residual){
              problem.AddResidualBlock(e, loss_function, para_PR[f]);
    ...
    if (!summary.IsSolutionUsable() || !std::isfinite(summary.final_cost)) {
      ROS_ERROR_STREAM("Ceres rejected the LIO update: "
                       << summary.BriefReport());
      return;
    }
    ...
    if (deltaR < 0.05 && deltaT < 0.05 || (iterOpt+1) == max_iters){
      ...
      if (lio_livox::numerics::ComputeRegularizedSqrtInformation<15>(
        ...
        ROS_WARN_THROTTLE(1.0,
                          "Skipping marginalization IMU factor with invalid covariance.");
      }
      ...
      for (auto &e : edgesLine[f]) {
        if(vLineFeatures[f][cntFtu].valid){
          auto *residual_block_info = new ResidualBlockInfo(e, loss_function,
