...
PCSeg::~PCSeg()
{
    ...
    // corPoints points into downsampledPoints_, which owns its memory.
    this->corPoints=NULL;
}
...
int PCSeg::DoSeg(int *pLabel1, float* fPoints1, int pointNum)
{
    ...
    if (pLabel1 == NULL || fPoints1 == NULL) {
        return -1;
    }

    // 1 down sampling. These buffers retain their capacity between scans.
    downsampledPoints_.resize(static_cast<size_t>(pointNum) * 4);
    pointVoxelIndices_.resize(pointNum);
    voxelPointIndices_.resize(pointNum);
    float *fPoints2=downsampledPoints_.data();
    int *idtrans1=pointVoxelIndices_.data();
    int *idtrans2=voxelPointIndices_.data();
    int pntNum=0;
    if (this->pVImg == NULL)
    {
        this->pVImg=(unsigned char*)calloc(DN_SAMPLE_IMG_NX*DN_SAMPLE_IMG_NY*DN_SAMPLE_IMG_NZ,sizeof(unsigned char));
    ...
    //先进行地面校正
    if (pntNum == 0) {
        std::fill(pLabel1, pLabel1 + pointNum, -1);
        corPoints = NULL;
        corNum = 0;
        return 0;
    }
    ...
    // 3 点云矫正
    if (this->CorrectPoints(fPoints2,pntNum,this->gndPos) != 0)
    {
        // A missing/invalid ground normal must not collapse the point cloud or
        // turn unknown points into dynamic objects. Let the caller keep this
        // scan unfiltered and try ground estimation again on the next scan.
        std::fill(pLabel1, pLabel1 + pointNum, 0);
        this->corPoints = NULL;
        this->corNum = 0;
        return -1;
    }
    this->corPoints=fPoints2;
    this->corNum=pntNum;

    // 4 粗略地面分割for地上分割
    groundLabels_.assign(pntNum, 0);
    int *pLabelGnd=groundLabels_.data();
    int gnum=GndSeg(pLabelGnd,fPoints2,pntNum,1.0);

    // 5 地上分割
    int agnum = pntNum-gnum;
    aboveGroundPoints_.resize(static_cast<size_t>(agnum) * 4);
    aboveGroundIndices_.resize(agnum);
    pointLabels_.assign(pntNum, 0);
    float *fPoints3=aboveGroundPoints_.data();
    int *idtrans3=aboveGroundIndices_.data();
    int *pLabel2=pointLabels_.data();
    int agcnt=0;//非地面点数量
    ...
    aboveGroundLabels_.assign(agcnt, 0);
    int *pLabelAg=aboveGroundLabels_.data();
    if (agcnt != 0)
    {
        AbvGndSeg(pLabelAg,fPoints3,agcnt);  
    }
    else
    {
...
int GetRTMatrix(float *RTM, float *v0, float *v1) // v0: ground normal, v1: target up axis
{
...
int PCSeg::CorrectPoints(float *fPoints,int pointNum,float *gndPos)
{
    ...
    if (GetRTMatrix(RTM,gndPos,znorm) != 0)
    {
        return -1;
    }
